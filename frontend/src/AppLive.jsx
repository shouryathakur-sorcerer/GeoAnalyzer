...
import WarAnalysisPanel from './components/WarAnalysisPanel.jsx';
...
export default function AppLive() {
  ...
  const [isWarPanelOpen, setIsWarPanelOpen] = useState(false);
  ...
  useEffect(() => {
    if (!selectedCountry || selectedIntel?.updatedAt) {
      return;
    }
  ...
  return (
    <div className="app-shell">
      ...
      <header className="glass-panel command-header">
        <div className="header-chip">Geo monitor</div>
        <h1 className="command-title">Global Intelligence Command</h1>
      ...
      <CommandDock
        ...
        onOpenWarAnalysis={() => setIsWarPanelOpen(true)}
        onSelectCountry={setSelectedCountry}
        onRemoveCountry={removeTrackedCountry}
        onClearAll={clearTrackedCountries}
      />
      ...
      {isWarPanelOpen && (
        <WarAnalysisPanel
          ...
          onClose={() => setIsWarPanelOpen(false)}
        />
      )}
    </div>
  );
